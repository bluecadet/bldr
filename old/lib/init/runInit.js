...
function RunInit(args) {
  ...
  this.run = async function() {
    ...
    if (!fs.existsSync(configPath)) {
      ...
      console.log(`Attempted to create default project config but ${path.basename(baseDir)} already contains a bldrConfig.js file.`);
    ...
    if (!fs.existsSync(localConfigPath)) {
      fsx.copy(dlc, localConfigPath, () => {
        console.log(`Local config copied to ${path.basename(baseDir)}`);
      })
    } else {
