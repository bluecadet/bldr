...
import { testBabelConfig } from '../utils/testConfig.js';
import { testSWCConfig } from '../utils/testConfig.js';
...
async function createRollupConfig(config) {
  ...
  rollupConfig.useSWC = userSettings && 'useSWC' in userSettings ? userSettings.useSWC : false;
  ...
  if ( !rollupConfig.overrideInputPlugins ) {
    ...
    if ( rollupConfig.useSWC ) {
      inputPlugins.push(swc(rollupConfig.swcOptions));
    } else if ( rollupConfig.useBabel ) {
