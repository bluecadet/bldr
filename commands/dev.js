...
export const RunBldrDev = async (commandOptions) => {
  ...
  const isSDC = configData?.processes?.singleDirectoryComponent;
  let SDC = false;
  ...
  if (commandOptions.settings?.once) {
    if (envKey) {
      handleProcessAction(
        'bldr',
        `Running single dev build using ${envKey} env configuration...`
      );
    } else {
  ...
  if (envKey) {
    handleProcessAction(
      'bldr',
      `Starting dev using ${envKey} env configuration...`
    );
  } else {
  ...
  if (!configData?.processSettings?.browsersync?.disable) {
    if (!configData?.local) {
      handleProcessWarn(
        'bldr',
        `Create a ${settings.localFilename} file in project root to configure browsersync`
      );
    }

    const bsLocalOpts = configData.local || {};
    const bsOptions = bsLocalOpts?.browserSync
      ? { ...bsLocalOpts.browserSync }
      : {};
    bsInstanceName =
      bsLocalOpts?.browserSync?.instanceName ??
      `bldr-${Math.floor(Math.random() * 1000)}`;
