...
export async function getConfigData(bldrCommand) {
  ...
  if (!existsSync(configPath)) {
    handleProcessError('bldr', `${settings.filename} does not exist.`, {
      throwError: true,
      exit: true,
    });
  }
  ...
  const config = {
    ...
    processes: {},
    processSettings: {
      rollup: rootConfig?.processSettings?.rollup
        ? rootConfig.processSettings.rollup
        : false,
      esBuild: rootConfig?.processSettings?.esBuild
        ? rootConfig.processSettings.esBuild
        : false,
      browsersync: rootConfig?.processSettings?.browsersync
        ? rootConfig.processSettings.browsersync
        : false,
      sass: rootConfig?.processSettings?.sass
        ? rootConfig.processSettings.sass
        : false,
      eslint: rootConfig?.processSettings?.eslint
        ? rootConfig.processSettings.eslint
        : false,
    },
  };
  ...
  if (bldrCommand.bldrEnv === 'dev') {
    ...
    if (bldrCommand.settings?.key) {
      ...
      if (targetDevProcessKey in devConfig) {
        ...
        handleProcessError(
          'bldr',
          `${targetDevProcessKey} is not a configured process key in config.`,
      ...
      if (devConfig?.env && targetDevEnvKey in devConfig.env) {
        ...
        handleProcessError(
          'bldr',
          `${targetDevEnvKey} is not a configured 'env' key in config.`,
          { throwError: true, exit: true }
        );
      }
    } else {
    ...
    if (bldrCommand.settings?.key) {
      ...
      if (targetBuildProcessKey in buildConfig) {
        ...
        handleProcessError(
          'bldr',
          `${targetBuildProcessKey} is not a configured process key in config.`,
          { throwError: true, exit: true }
        );
      }
    } else {
  ...
  // if (rootConfig?.singleDirectoryComponent?.baseDirectory) {
  //   config.processes.singleDirectoryComponent =
  //     rootConfig.singleDirectoryComponent;
  // }
