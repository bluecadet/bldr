...
async function build(fileName, inputOptions, outputOptions) {
  let bundle;
...
async function handleRollupFile(config, file) {
  ...
  if (
    ...
    fs.existsSync(path.join(rootPath, '.eslintrc.cjs')) ||
    fs.existsSync(path.join(rootPath, '.eslintrc.yaml')) ||
    fs.existsSync(path.join(rootPath, '.eslintrc.json'))
  ) {
    inputPlugins.push(eslint());
  }
  ...
  await build(fileName, inputOptions, outputOptions);
}
