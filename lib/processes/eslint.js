...
async function handleEsLintFiles(fileArray, config) {
  ...
  try {
    ...
    const allowErrors = config?.isWatch ? true : false;
    const results = await eslint.lintFiles(fileArray, {warnIgnored: allowErrors});
  ...
  return new Promise((resolve, reject) => {
    testAllowRun(config)
      .then(allow => {
        if (!allow) {
          if ( config?.eslint?.omit ) {
            handleMessageSuccess('eslint', 'process not ran: eslint ignored in bldrconfig');
          } else {
